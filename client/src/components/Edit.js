...
import {get_all_alumni} from '../api_calls/get_all_alumni.js'
...
export default function Edit() {
  ...
  const modeMapping = {
    "team": {name: "Team Member", structure: teamDataStructure, pull: get_all_team}, 
    "project" : {name: "Project", structure: projectDataStructure, pull: get_all_project}, 
    "banner" : {name: "Banner", structure: bannerDataStructure, pull: get_all_banner}, 
    "alumni" : {name: "Alumni", structure: alumniDataStructure, pull: get_all_alumni}
  }
  ...
  useEffect(()=> {
    ...
    modeMapping[currentMode].pull(setTempData, setTempMode)
  ...
  useEffect(()=> {
      if (data.length !== 0 && switchMode) {
          ...
          setCurrDataSturcture((modeMapping[currentMode].structure));
          setSwitch(false)
      }
  ...
  return (
    ...
    <h1> Edit {modeMapping[currentMode].name}</h1>
        <select value={currentMode} onChange={(e)=>{setCurrentMode(e.target.value)}}>
            ...
            <option value="alumni">Edit Alumni</option>
        </select>
    </div>
    ...
    <div class="selectContainer">
        <p class="selectPara"> Select {modeMapping[currentMode].name}:</p>
        <select class="select" value={select} onChange={(e) => {setSelect(parseInt(e.target.value))}}>
            {data.map((o, index) => {
                return (
                    <option value = {index}> {(currentMode === "project") ? o.title : o.name} </option>
                )
            })}
        </select>
    </div>
